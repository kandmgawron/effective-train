...
export default function TemplateBuilder() {
  ...
  const handleAddExercise = (exercise: Exercise) => {
    setSelectedExercises(prev => [...prev, {
      id: Date.now(), templateId: 0, exerciseId: exercise.id,
  ...
  const handleRemoveExercise = (removeId: number) => {
    setSelectedExercises(prev => {
      const updated = prev.filter(ex => ex.id !== removeId);
  ...
  const handleSaveTemplate = () => {
    ...
    const result = db.runSync('INSERT INTO workout_templates (name, estimated_duration) VALUES (?, ?)',
      [templateName, totalTime]);
    ...
    cleaned.forEach((ex, i) => {
      db.runSync('INSERT INTO template_exercises (template_id, exercise_id, sets, target_reps, rest_time, exercise_order, superset_group) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [templateId, ex.exerciseId, ex.sets, ex.targetReps, ex.restTime, i, ex.supersetGroup ?? null]);
    });
    router.back();
  };
