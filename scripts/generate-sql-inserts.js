...
for (let i = 1; i < lines.length; i++) {
  ...
  sqlStatements += `      db.runSync('INSERT INTO exercises (name, body_part, equipment, instructions, is_custom) VALUES (?, ?, ?, ?, 0)', ['${name}', '${bodyPart.replace(/'/g, "''")}', '${equipment.replace(/'/g, "''")}', '${instructions.replace(/'/g, "''")}']);\n`;
  count++;
}

console.log(`Generated ${count} SQL insert statements`);
fs.writeFileSync('lib/exercise-inserts.txt', sqlStatements);
console.log('Saved to lib/exercise-inserts.txt');
...
function mapEquipment(equipment) {
  ...
  if (lower.includes('exercise ball')) return 'Exercise Ball';
